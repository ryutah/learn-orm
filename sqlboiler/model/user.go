...
func (o *User) Emails(mods ...qm.QueryMod) emailQuery {
	...
	queryMods = append(queryMods,
		qm.Where("`email`.`user_id`=?", o.ID),
	)

	query := Emails(queryMods...)
	queries.SetFrom(query.Query, "`email`")

	if len(queries.GetSelect(query.Query)) == 0 {
		queries.SetSelect(query.Query, []string{"`email`.*"})
	}

	return query
}

// LoadEmails allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (userL) LoadEmails(ctx context.Context, e boil.ContextExecutor, singular bool, maybeUser interface{}, mods queries.Applicator) error {
	var slice []*User
	var object *User
	...
	query := NewQuery(qm.From(`email`), qm.WhereIn(`email.user_id in ?`, args...))
	if mods != nil {
		mods.Apply(query)
	}
	...
	if err = results.Close(); err != nil {
		return errors.Wrap(err, "failed to close results in eager load on email")
	}
	...
	for _, foreign := range resultSlice {
		for _, local := range slice {
			if local.ID == foreign.UserID {
				local.R.Emails = append(local.R.Emails, foreign)
				if foreign.R == nil {
					foreign.R = &emailR{}
				}
...
// AddEmails adds the given related objects to the existing relationships
// of the user, optionally inserting them as new records.
// Appends related to o.R.Emails.
...
func (o *User) AddEmails(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Email) error {
	...
	for _, rel := range related {
		if insert {
			...
			values := []interface{}{o.ID, rel.Email}
	...
	if o.R == nil {
		...
		o.R.Emails = append(o.R.Emails, related...)
	}
